...
| **Catalogue derivation** (T04) | **PASS — 0.30–0.45 s** wall for the stdlib `json` parse of the 51.3 MiB bundle in a fresh warm-cache process (0.7–0.9 s inside the full test-suite process), **peak working set 274 MiB** by `GetProcessMemoryInfo` against a 15 MiB bare-interpreter baseline, `tracemalloc` peak 257 MiB. End-to-end derive — parse, project, sha256 the bundle, write — 0.39–0.43 s, so `D-03`'s build-time derivation is affordable on stdlib `json` alone. Three slices read **from the bundle, not from memory**: `T1059.001` → "PowerShell", `execution`; `T1078` → "Valid Accounts", `stealth persistence privilege-escalation initial-access`; `T1569.002` → "Service Execution", `execution` **only** — both tactic facts §4's `R4.4`–`R4.9` paragraph rests on hold in v19.2, so that paragraph needs no correction; `T1068` is present and selectable. `attack_version` reads exactly `19.2` from the one `x-mitre-collection` object. Deleting `catalogue_v19.2.json` and regenerating is byte-identical at **1,475,013 bytes**: `D-03`'s ~200 KB estimate describes the projection *without* descriptions (262,248 bytes), and the 858 descriptions are 1,185,188 characters that stage 4 retrieves candidates against, so they are kept whole — a 36x reduction from the bundle, where the descriptionless projection that estimate describes would have been 205x. One upstream change worth pinning: v19.2 renames `TA0005` to **Stealth** and adds Defense Impairment (`TA0112` / `defense-impairment`), so `defense-evasion` is no longer a tactic shortname at all — a hand-written mapping table from older ATT&CK would have missed silently |
